...
// Định nghĩa 4 LED tích hợp trên STM32F407 Discovery
#define LED_GREEN   PD12
#define LED_ORANGE  PD13
#define LED_RED     PD14
#define LED_BLUE    PD15

// Khởi tạo con trỏ HardwareTimer liên kết với TIM2
HardwareTimer *MyTim = nullptr;

// Cờ báo hiệu ngắt (volatile để tránh compiler tối ưu sai lệch)
volatile bool timerFlag = false;

// Hàm ISR (Interrupt Service Routine) được gọi khi timer tràn
void Timer_ISR(void) {
  // Đảo trạng thái các LED trực tiếp trong ngắt
...
void setup() {
  ...
  // Khởi tạo Timer phần cứng TIM2
  MyTim = new HardwareTimer(TIM2);

  // Tạm dừng timer trước khi cấu hình
  MyTim->pause();

  // Cấu hình chu kỳ ngắt: 500,000 microseconds (0.5 giây)
  // Tương đương tần số 2 Hz
  MyTim->setOverflow(500000, MICROSEC_FORMAT);

  // Gắn hàm callback xử lý ngắt
  MyTim->attachInterrupt(Timer_ISR);

  // Kích hoạt Timer chạy
  MyTim->resume();
}

void loop() {
  // CPU hoàn toàn rảnh rỗi trong loop() vì ngắt chạy độc lập ở phần cứng
  if (timerFlag) {
    timerFlag = false;
    // Có thể thực hiện các tác vụ nặng không ưu tiên tại đây nếu cần
  }
}